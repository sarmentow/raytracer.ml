...
let (-:) name f = Alcotest.test_case name `Quick f
  ...
  let tests = "chapter2", [
    ...
    "create canvas" -: begin fun () ->
      ...
      Alcotest.(check bool) "canvas has proper dimensions and all pixels are black initially" true 
        ((Canvas.w canvas = 10) && 
        (Canvas.h canvas = 20) &&
        (Array.for_all 
          (fun row -> (
    ...
    "write pixels to canvas" -: begin fun () ->
      ...
      Alcotest.(check bool) "red pixel at (5, 5)" true 
    ...
    "write canvas to ppm" -: begin fun () ->
      ...
      Alcotest.(check bool) "outputs expected ppm header" true
      (String.starts_with (Buffer.contents ppm)  ~prefix:"P3\n5 3\n255")
    end;
    "write pixels to canvas and scales to ppm correct values" -: begin fun () ->
      let canvas = Canvas.init 5 3 in
      ...
      Alcotest.(check bool) "outputs expected ppm file" true
      (
        match lines with
    ...
    "write pixels to ppm and breaks lines  to be below 70 chars" -: begin fun () ->
    ...
    "ppm files are terminated by a newline" -: begin fun () ->
      let canvas = Canvas.init_with_color 10 2 (Color.init 1. 1. 1. 1.) in
      let ppm = Canvas.canvas_to_ppm canvas in
      let b = Buffer.contents ppm in
      let lines = (String.split_on_char '\n' b) in
      (* Printf.printf "%s" b; *)
      Alcotest.(check bool) "ends in newline" true
      (
